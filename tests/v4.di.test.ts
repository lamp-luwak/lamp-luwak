...
test("should throw exception with plain values", () => {
  const d = new Date();
  const c = {};
  expect(() => service(null as any)).toThrowError("Only function and class supported");
  expect(() => service("hello" as any)).toThrowError();
  expect(() => service(10 as any)).toThrowError();
  expect(() => service(d as any)).toThrowError();
  expect(() => service(c as any)).toThrowError();
});
